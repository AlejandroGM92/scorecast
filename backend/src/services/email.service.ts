...
export async function verifySmtp(): Promise<{ ok: boolean; error?: string }> {
  if (!process.env.RESEND_API_KEY) {
    return { ok: false, error: 'RESEND_API_KEY no está configurado en Render' };
  }
  // Resend uses HTTP API — no TCP connection to verify, just check key exists
  return { ok: true };
}
...
export async function sendMatchReminderEmail(params: {
  ...
  if (!process.env.RESEND_API_KEY) {
    logger.warn('📧 Email skipped — RESEND_API_KEY not set in environment variables');
    return false;
  }
  ...
  const from = process.env.RESEND_FROM || 'SCORECAST <onboarding@resend.dev>';
